...
def main() -> int:
    ...
    # per-utility, per-tier detail behind the vintages table (every MW figure quoted in the text): CEC statewide vintages in
    # table A, the utility-level series (SCE database, PG&E pipeline) plus memo rows in table B
    ...
    for (v, lab), g in det.groupby(["vintage", "label"], sort=True):
        target = rows_b if ("SCE database" in lab or "PG&E earnings" in lab) else (rows_c if "Dec 2025" in lab else rows_a)
