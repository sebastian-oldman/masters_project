...
summ[cols].round(3)"""),
    ...
    md("""**Reading.** CAISO annual demand has been flat at roughly 214-224 TWh since 2019 while utility-scale solar and wind rose from a fifth to nearly a third of demand and net imports fell from a quarter of demand to a tenth (2024). The hourly peak record remains September 6, 2022 (51.1 GW)."""),
...
CAISO's Today's Outlook reports 5-minute CO2 by source including imports (metric tons per hour). Intensity here is total CO2 divided by CAISO demand, in g CO2 per kWh, energy-weighted per year. eGRID's CAMX annual output emission rate is the reference annual factor (generation-based, 2023)."""),
    ...
    md("""**Reading.** Load-based intensity fell from about 257 g/kWh in 2019 to about 195 g/kWh in 2025, with spring midday hours near 50 g/kWh and night hours near 240 g/kWh. eGRID's 2023 CAMX output rate (198 g/kWh) is lower than the CAISO load-based 2023 figure (242 g/kWh) because it is generation-based, excludes the default import factor and covers the whole CAMX subregion; the trend is the same. Battery discharge grew from 0.05 TWh in 2019 to 12.3 TWh in 2025 with a 10.7 GW peak, which is what moved the net-peak hours later in the evening."""),
