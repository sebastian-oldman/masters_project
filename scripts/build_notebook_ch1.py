...
The four items are not four comparable estimates of one quantity. A is the only statewide historical estimate (EPRI, 2023); B converts the CEC's ~1,000 MW existing peak with an assumed annual load factor; C is an illustrative count-based sensitivity (facility points x an assumed per-facility energy taken from the SVP cluster, no peak or utilization factor); D is a single-utility subset. Shares use EIA-861 retail sales (Parts A + C + D) of the matching year."""),
    ...
    md("""## 5. The current status in the workshop format (2025-2026)

The five figures below repeat, for CAISO, the charts Jukka Manner used for Finland at the IRTF SUSTAIN workshop (July 2026): hourly consumption against generation, hourly prices, hourly emission factors, monthly imports and exports, and wind (here also solar) against installed capacity. Sources: EIA-930 (2026 hourly balance and neighbour-level interchange), CAISO OASIS (day-ahead prices), CAISO Today's Outlook (CO2 and fuel mix) and EIA-860M July 2026 (installed capacity in the CAISO balancing authority)."""),
    code("""ws = table("ch1_workshop_stats"); ws.round(1)"""),
    code("""show("fig1_09_consumption_2026")"""),
    code("""show("fig1_10_prices_2025_2026")"""),
    code("""show("fig1_11_emissions_2026")"""),
    code("""ie = table("ch1_imports_exports_monthly"); ie.round(0)"""),
    code("""show("fig1_12_imports_exports_monthly")"""),
    code("""table("ch1_installed_wind_solar_2026_07")"""),
    code("""show("fig1_13_wind_solar_2026")"""),
    md("""**Reading.** In 2026 to date CAISO demand ran between the minimum and maximum in the table above, with generation inside CAISO covering about 70 percent of it and net imports the rest; the day-ahead price averaged about 32 to 37 USD/MWh at the three DLAPs (3 to 4 US cents per kWh, wholesale only) with negative hours in spring; the consumption-side CO2 intensity averaged about 160 g/kWh and the production-side factor about 115 g/kWh; gross imports were 35 TWh in 2025 against 5 TWh of exports; wind ran at about a quarter of its 10 GW nameplate and solar at a quarter of its 26 GW nameplate on average. The exact values are in `ch1_workshop_stats.csv`."""),
    md("""## 6. Institutional context (documented in `report/sections/02_current_status.tex`)
