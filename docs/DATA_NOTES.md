...
## Communication package for the advisor (2026-09-22)

The advisor asked for the work in a form he can read without raw data, preferably an evolving Overleaf project. Added: a one-page "Summary for the Reader" with a glossary at the front of the report (`report/sections/00a_summary.tex`; report now 91 pages, 0 errors); a progress memo answering his inline questions with corrected numbers (`report/brief/advisor_brief_2026-09-22.tex`, `make brief`; every number in it traced to a figure, table or processed file); and `make overleaf` (= `scripts/export_overleaf.py`), which packages the report, the memo and the deck as one Overleaf-ready zip (`release/overleaf_consumption_gap_2026-09-22.zip`) with figures and bibliography shared and graphicspath rewritten. Corrections carried in the memo: the CAISO record peak was set on September 6 2022 (not October 2023); the "6 GW midday, 22 GW evening" net-load statement is the spring profile (all-season means 15.3 to 10.1 GW midday, 26.4 to 25.6 GW at 20:00); the "6,771 to 23,277 MW" comparison mixes tier sets and utilities (like for like, agreements plus applications at PG&E and SCE, 6,771 to 11,219 MW); 2025 midday prices 13 (SCE) to 24-26 (PG&E) USD/MWh and evening 49-56. The Oregon comparison the advisor raised (ECOnorthwest and University of Virginia, September 2026: 111 data centers, about 23 percent of Oregon retail sales in 2025, nearly 25 TWh expected by 2030) is registered as `opb_oregon_data_centers_2026_09` (OPB article; the report PDF itself is not in the store) and is not used in the report.

## Advisor memo fact-check (2026-09-22, second pass)

Every statement in the memo was re-checked against the processed files and the primary sources before the memo was regenerated by `scripts/make_advisor_brief.py`, which embeds the ten report figures it refers to (hyperlinked within the memo) and attaches a clickable source link, taken from the manifest by source id, to each claim. Corrections made: (1) the variance attributed to the three tier probabilities is 2 percent of the energy gap and 4 percent of the peak gap (Sobol total indices 0.008 + 0.009 + 0.001 and 0.018 + 0.021 + 0.002), not "about 3 percent"; fixed in chapter 5, the discussion, the deck and notebook 04. (2) QCEW 518210 employment is described as in chapter 3: 12 percent a year to 74,000 by the end of 2022, a step to 84,000 in 2023 Q1, flat since (84,000 in 2026 Q1); "flat since 2022" alone was loose. (3) The Oregon "more than 14 TWh" was not in the OPB article; the memo now quotes OPB (111 facilities, about 23 percent of retail sales in 2025, nearly 25 TWh by 2030) and derives 14 TWh from Oregon's 59.7 TWh of 2024 retail sales (EIA-861, same A+C+D method as chapter 1; California 245.7 TWh), labelled as derived. (4) The RQ1 table is Table 3.10 and the SCE database table is Table 3.9 (checked with pdftotext). (5) The energy share of the flat upper bound is 81 to 84 percent of CAISO's 216 to 224 TWh. (6) Epoch's US sites are led by Texas, Virginia, Ohio and Iowa by count (12, 7, 5, 4; 15 addresses without a parseable state); "Arizona" was dropped. (7) The claim that CAISO's markets extend into other states was dropped for lack of a source in the store. Balancing authorities other than CAISO are cited from the CED forms (LADWP, BANC, TID) and EIA-930 codes (IID, PACW, WALC); the 80 percent share is 212,700 of 263,196 GWh and 46,479 of 57,560 MW in the CED 2025 Planning forms.

## Workshop-format figures (2026-09-22): the Finnish slides, the Census chart and the Oregon share, for California

The advisor's material (Manner's IRTF SUSTAIN slides of July 2026, Politano's Census construction chart, the ECOnorthwest/UVA Oregon report as covered by OPB and Mashable, and the E&E News Pallone article) was compared with the report's figures. The Census chart already existed (fig2_01, same C30 series with the ChatGPT marker); the Pallone article is policy text with no chart. Nine California figures were added so that every chart in the slides and the Oregon share have a counterpart; the Finland-versus-California bar chart was not built (California-only scope). No new raw source was needed: every input was already in the manifest (EIA-930 balance and interchange 2025-2026, CAISO Today's Outlook 2026, OASIS 2025-2026, EIA-860M July 2026, the SCE public database TN268459, the CED 2025 forms, EPRI and SVP). The manifest and the freeze are unchanged.

| Figure | Content | Data | Code |
|---|---|---|---|
| fig1_09 (report Figure 2.9) | hourly CAISO demand and in-CAISO net generation, Jan 1 to Sep 20 2026, with min/max/mean | `ch1_hourly_2026_workshop.csv`, `ch1_workshop_stats.csv` | `run_chapter1.py` step 5 |
| fig1_10 (2.10) | hourly day-ahead prices at the three DLAPs, Jan 2025 to Sep 22 2026, USD/MWh and cents/kWh | `caiso_dam_lmp_hourly.parquet`, `ch1_workshop_stats.csv` | step 5 |
| fig1_11 (2.11) | hourly CO2 intensity 2026: consumption (accounting) and a new in-CAISO production factor (`ch1_baseline.production_intensity`) | `ch1_carbon_intensity_hourly_2026.csv` | step 5 |
| fig1_12 (2.12) | gross imports and exports by month, Jan 2025 to Aug 2026, from the neighbour-level EIA-930 interchange (`load_eia930_interchange_ciso`); complete months = at least 95 percent of hours | `ch1_imports_exports_monthly.csv`, `ciso_interchange_gross_2025_2026.parquet` | step 5 |
| fig1_13 (2.13) | hourly wind and solar 2026 against EIA-860M July 2026 nameplate in the CAISO balancing authority (`eia860m_ba_capacity`) | `ch1_installed_wind_solar_2026_07.csv` | step 5 |
| fig2_05 (3.5) | current capacity (SVP cluster 26 percent of the 2023 statewide estimate) and statistics of the projects by stage with ratios; largest SCE request (`ch2_growth.sce_largest_requests`) | `ch2_project_statistics.csv`, `ch2_sce_largest_requests.csv` | `run_chapter2.py` step 6 |
| fig2_06 (3.6) | annual energy of the requests in electric-car years (3 MWh per car-year, Manner's assumption, registered in the assumption register) | `ch2_ev_equivalents.csv` | step 6 |
| fig3_06 (4.6) | Fingrid-style stacked production by technology: EIA-923 2025 and the three 2030 cases | `ch3_production_projection.csv` | `run_chapter3.py` step 5 (`workshop_figure`, re-runnable alone with `--workshop-only`) |
| fig4_08 (5.8) | data centers as a share of California's electricity, 2023-2040: history over EIA-861 retail sales, CED 2025 Planning and Local Reliability (existing 7.0-9.3 TWh plus the forecast additions, over statewide energy to serve load), 2030 bounds | `ch4_dc_share_trajectory.csv` | `run_chapter4.py` step 9 |

**Headline numbers (all in the figures or the new tables ch1_workshop_stats, ch1_imports_exports_monthly, ch1_installed_wind_solar, ch2_project_statistics, ch2_ev_equivalents, ch3_production_projection, ch4_dc_share):** 2026 demand 18,748-49,959 MW (mean 27,202) over 6,301 valid hours, in-CAISO net generation mean 19,329 MW, net imports mean 4,405 MW; DLAP prices mean 36.8/32.0/35.3 USD/MWh over 15,119 hours; consumption CO2 factor mean 162.6 g/kWh (range -128.9 to 395.2), production factor 116.4 (13.5 to 356.9); 2025 gross imports 42.0 TWh, exports 6.0 TWh, net 35.9 TWh over 8,712 hours (chapter 1 balance-file net 36.1 TWh; hourly check over 14,949 hours, mean |diff| 1 MW, r = 1.0000); wind 9,994 MW nameplate in CISO (6,479 MW in California), 2026 mean 2,582 MW (26 percent), max 7,126 MW; solar 25,810 MW, mean 6,548 MW (25 percent), max 20,852 MW; SVP cluster 2.46 of 9.33 TWh (26 percent, 25-29); largest SCE request 1,000 MW (Mojave, active application, 2026); EV equivalents 2.9 / 14.9 / 60.4 million cars flat (1.7 / 8.8 / 35.6 million at the CEC case); production 207.1 TWh in 2025 and 236.3 / 230.8 / 204.1 TWh in the 2030 cases; data center share 3.9 percent (2023), 6.6 percent 2030 and 10.1 percent 2040 (Planning), 11.3 and 12.5 percent (Local Reliability), 40.4 percent (flat bound) and 29.1 percent (CEC utilization bound) in 2030; Oregon 23 percent (2025) and 31-32 percent (2030) in the caption for comparison.

**Checks:** report 104 pages, 0 errors, 0 overfull, 0 undefined references; a machine check of the four new sections found all 171 quantified numbers in a figure text layer, a new table or a cited source (Manner 2026 and OPB 2026 values); provenance appendix rebuilt (245 manifest ids behind the figures, all labels resolve); assumption register 20 rows (two added: the existing-energy band of the share trajectory and the electric-car assumption); notebooks 01-05 re-executed with the new cells; defense deck 23 slides (two added); advisor memo 11 pages with report Figure 5.8 and its sources linked; release and Overleaf exports refreshed. The bundle received the four EIA-930 interchange files (2025-2026, 354 MB), the SVP Assembly hearing deck that fig2_05 names, and their manifest rows (bundle manifest 283 ids, re-frozen); its `run_all.sh` now runs the freeze before the provenance appendix so the appendix reads the current subset. Bundle rebuilt from raw: all 33 PNGs byte-identical, processed CSV/JSON numerically identical, LaTeX tables identical.

**Caveats stated in the text:** wholesale prices are not comparable in level with Finland's consumer price with taxes; the production factor uses CAISO's accounting CO2 of in-CAISO sources over the fuel-mix generation excluding imports and batteries; the share trajectory mixes denominators (retail sales for history, energy to serve load for the forecast) and states the basis per row; the electric-car figure is an assumption used for presentation only.
